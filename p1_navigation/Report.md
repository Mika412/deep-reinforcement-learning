...
In the paper, the input is an image, so the network uses a CNN. For this project, the parameters are provided by the environment in an vectorized form, so a deep neural network with 3 fully connected layer is used.
...
![results](p1_navigation/scores.png)
